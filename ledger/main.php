<?php
  ...
  class NXTBridgeWalletPage {
    ...
    public function __construct() {
      ...
      $this->api = "//api.nxter.org/v2";
    }

    /*****************************************************************************************************************/

    public function show_ledger($account) {
    ?>
    <table class='nb table nxtbridge-table'>
      <thead class='ledger-header'><tr>
        <td>Date</td><td>Type</td><td>Change</td><td>Balance</td><td>From / To</td>
      </tr></thead>
      <?php
        $proto = "http:"; if ( is_ssl() ) { $proto = "https:"; } 
        $url = $proto . $this->api . "/peer" ; 
        ...
        $url = "http://" . $random_node . ":80" . "/nxt?requestType=getAccountLedger&account=" . $account . "&includeHoldingInfo=true&includeTransactions=true&firstIndex=0&lastIndex=14"; 
        $tmp = file_get_contents( $url );
          ...
          for ( $i=0; $i<count($ledger); $i++ ) {
            ...
            switch ( $type ) { 
              case 'ASSET_TRANSFER':
                ...
                $change = $ledger[$i]['change'] / pow(10, 8 - $asset_dec); $change = round($change,$RND) . ' ' . $asset;
                $balance = $ledger[$i]['balance'] / pow(10, 8 - $asset_dec); $balance = round($balance,$RND) . ' ' . $asset;
              ...
              case 'TRANSACTION_FEE':
                $change = $ledger[$i]['change'] / pow(10, 8); $change = round($change, $RND) . ' NXT';
                $balance = $ledger[$i]['balance'] / pow(10, 8); $balance = round($balance, $RND) . ' NXT';
              ...
              case 'CURRENCY_EXCHANGE_SELL':
                ...
                $change = $ledger[$i]['change'] / pow(10, 8 - $cur_dec); $change = round($change, $RND) . ' ' . $cur;
                $balance = $ledger[$i]['balance'] / pow(10, 8 - $cur_dec); $balance = round($balance, $RND) . ' ' . $cur;
              ...
              case 'ORDINARY_PAYMENT':
                ...
                if ( isset($ledger[$i]['transaction']['attachment']['message'] ) ) {
                  $type_p = $type_p . "<br><em>Message: " . $ledger[$i]['transaction']['attachment']['message'] . "</em>";
                }

                if ( isset($ledger[$i]['transaction']['attachment']['encryptedMessage'] ) ) {
                  $type_p = $type_p . "<br><em>Encrypted Message.</em>";
                }
              ...
              case 'ASSET_DIVIDEND_PAYMENT':
                $change = $ledger[$i]['change'] / pow(10, 8); $change = round($change, $RND) . ' NXT';
                $balance = $ledger[$i]['balance'] / pow(10, 8); $balance = round($balance, $RND) . ' NXT';
                $type_p = 'Dividend payment';
                
                if ( isset($ledger[$i]['transaction']['attachment']['message'] ) ) {
                  $type_p = $type_p . "<br><em>Message: " . $ledger[$i]['transaction']['attachment']['message'] . "</em>";

                  $recipient = $ledger[$i]['accountRS'];
                  if ( $recipient == $account ) { $recipient = 'You'; }
    ...
    public function send_broadcast($tr) {
      if ( isset( $tr ) && strlen( $tr )>24 ) {
        $proto = 'http:'; if ( is_ssl() ) { $proto = 'https:'; } 
        ...
        if ( $nxt_node ==  false ) { 
          ...
          $random_node  = $nxt_node['data'];
          $url = 'http://' . $random_node . ':80/nxt';
          $data = array('requestType' => 'broadcastTransaction', 'transactionBytes' => $tr );
        ...
        $user_message = 'Empty signed transaction. Sorry.';
      }
...
/*========================================================================================================================*/
    public function send_NXT($recipient,$amount,$account,$message) {
      if ( isset($recipient) && isset($amount) && isset($account) && isset($message) ) {
        $proto = 'http:'; if ( is_ssl() ) { $proto = 'https:'; } 
        ...
        if ( $nxt_node ==  false ) { 
          ...
          $random_node  = $nxt_node['data'];
          $url = 'http://' . $random_node . ':80' . '/nxt?requestType=getAccountPublicKey&account=' . $account; 
          ...
          if ( isset($pub_key['publicKey']) ) { 
            $url = 'http://' . $random_node . ':80/nxt';
            $data = array('requestType' => 'sendMoney', 'recipient' => $recipient, 'amountNQT' => $amount * 100000000, 'publicKey' => $pub_key['publicKey'], 'feeNQT' => 100000000, 'deadline' => 720, 'broadcast' => 'false', 'message' => $message ) ;
            ...
            if ( $rr_descr ) {
              // FIX BUG: 1.2.6
              // set user_message to response by default
              $user_message = $rr;
              ...
              if ( $rr_descr['unsignedTransactionBytes'] ) { 
                ...
                '<div class="nb row">' .
                  ...
                  '<div class="nb col-xs-12 col-sm-6">' .
                  '<div class="nb input-group">' .
                    '<span class="nb input-group-addon">Recipient:</span>' .
                    '<input type="text" class="nb form-control" id="eRecipient" value=" '.$recipient.' " readonly></input>' .
                  '</div>'.
                  '</div>' .

                  '<div class="nb col-xs-12 col-sm-4">' .
                  '<div class="nb input-group">' .
                    '<span class="nb input-group-addon">' . 'Amount: ' . '</span>' .
                    '<input type="text" class="nb form-control" id="eAmount" value=" '.$amount.' " readonly></input>' .
                  '</div>' .
                  '</div>' .
                ...
                '<div class="nb row">' .
                  ...
                  '<div class="nb col-xs-12 col-sm-12">' .
                  '<div class="nb input-group">' .
                    '<span class="nb input-group-addon">' . 'Message: ' . '</span>' .
                    '<input type="text" class="nb form-control" id="eMessage" value=" '.$message.' " readonly></input>' .
                  '</div>' .
                  '</div>' .
                ...
                '<div class="nb input-group">' .
                  '<span class="nb input-group-addon" id="lunsignedTransaction">' . 'UnsignedTransaction: ' . '</span>' .
                  '<textarea class="nb form-control" rows="4" id="unsignedTransaction" readonly>' . $rr_descr['unsignedTransactionBytes'] . '</textarea>' .
                '</div><br>' .

                '<div class="nb btn-group">' .
                  '<a href="https://bitbucket.org/scor2k/nxtbridge-offline/downloads/index.min.html" class="nb btn btn-primary" id="DownloadNXTBridgeOffline"><i class="fa fa-download" aria-hidden="true"></i> Download NXTBridge Offline</a>' .
                  '<a href="#" class="btn btn-clipboard btn-primary" data-clipboard-target="#unsignedTransaction" id="CopyUnsignedTransaction"><i class="fa fa-files-o" aria-hidden="true"></i> Copy Unsigned Transaction</a>' .
                    '<script>' .
                      'new Clipboard(".btn-clipboard");' .
                    '</script>' .
                  '<a href="https://www.nxter.org/nxtbridge/" target=_blank class="nb btn btn-primary" id="Help"><i class="fa fa-question-circle-o" aria-hidden="true"></i></a>' .
                '</div>';
                } // end unsignedTransactionBytes
          ...
          } else { // end isset public key
            // FIX BUG 1.2.6 if public key is empty
            $user_message = '<i>Your account does not have public key!</i>';
            $this->response = $user_message;

          }

        } // end else if node exist
            
        $this->response = $user_message;
      } else { // first if
        $user_message = 'Empty params for send NXT';
        $this->response = $user_message;
	  }
...
/*========================================================================================================================*/
    ...
    public function create_user_page() {
        ...
        case 'NXTBridge_send':
          ...
          if ( isset($_POST['recipient']) && isset($_POST['amount']) ) {
            ...
            if ( $nxt_node ==  false ) { 
              ...
              $random_node  = $nxt_node['data'];
              $url = 'http://' . $random_node . ':80' . '/nxt?requestType=getAccountPublicKey&account=' . $this->options['account']; 
      ...
      <div class='wrap'>
        <h1>NXTBridge Ledger [<?php echo $this->login; ?>]</h1>
